...
for (int i = 0; i < filename.length; i++) {
	...
	try {
		bw = new BufferedWriter(new FileWriter(dataDirPath+"/"+"output"));
	...
	try {
		reader = new FileReader(new File(folder, filename[i]));
...
public void addToWordTable(int mode){
	...
	FileReader reader = null;
	try {
		reader = new FileReader(dataDirPath+"/"+"output");
		} catch (FileNotFoundException e2) {
		// TODO Auto-generated catch block
		e2.printStackTrace();
		}
...
public void save(){
	...
	try {
		hos = new FileOutputStream(dataDirPath+"/"+"hamdb");
		...
		FileOutputStream spos = new FileOutputStream(dataDirPath+"/"+"spamdb");
		...
		FileOutputStream aros = new FileOutputStream(dataDirPath+"/"+"arcdb");
		ObjectOutputStream arout = new ObjectOutputStream(aros);
		arout.writeObject(arcsfinal);
		arout.close();
		aros.close();
		
		FileWriter fw = new FileWriter(dataDirPath+"/"+"stats");
